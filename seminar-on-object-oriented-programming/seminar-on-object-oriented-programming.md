...
0:00[Music]
0:03good idea to put up a slide warning you
0:07there might be some IBM jokes of course
0:10this isn't I guess this isn't really IBM
0:12country out here although I did hear a
0:15rumor that you're still programmed in
0:19Fortran quite a bit and even the dread
0:22word COBOL was heard so maybe it's
0:25worthwhile
0:26you know object-oriented programming is
0:28actually a rather old technology it is
0:34just a few years younger in fact than
0:37the kind of programming technology that
0:40most people are used to that of data
0:43structures manipulated by procedures and
0:46for various reasons it has not come into
0:50currency into vogue until the last
0:52couple of years but it's actually about
0:5525 years ago I wrote my first program
0:58that I would call object-oriented in
1:001962 and people have been writing
1:04programs in that style even earlier than
1:08that in my particular case I was a
1:11programmer in the Air Force at Air
1:15Training Command in 1962 the there were
1:19no standardized operating systems of
1:22course that's true today
1:25but back then there was actually a
1:27reason for it
1:29nobody had gotten around to the idea of
1:31thinking it it might be useful and Air
1:33Training Command had a problem of moving
1:38tapes tapes with records of various
1:43kinds on them from one Air Training
1:45Command base to another the machines
1:49back borrows two twenties which were
1:53machines that have had about five
1:55thousand characters of of storage we're
1:58about the size of this room and we're
1:59great coffee warmers my image of them of
2:04the 220 is with a this huge set of boxes
2:07with a row of pots on top trapping the
2:12heat is a came out now the problem that
2:17they had was that there is no way to
2:20standardize on what the formats of the
2:22different records were they changed and
2:25the operating system he was usually just
2:28the kind of a loading program and so
2:31there was no way of standardizing on
2:32that and they wondered how can we
2:35actually read these data records if
2:38everything is constantly changing and
2:40nothing holds still somebody thought up
2:42a great idea that idea was to have the
2:48records shipped in the following format
2:51the each tape file had three sections to
2:55it the front section was fairly short it
2:58had a bunch of relative pointers into
3:01the second section which contained a
3:03bunch of Burroughs 220 code and that
3:07code knew how to deal with a third
3:08section which had the actual data
3:10records and so what you got when you
3:15read a record from this machine was you
3:20read that record the first two parts of
3:23the of the record into storage use the
3:29the front part of it which were these
3:32indirect jumps into the second part of
3:34it the code as the protocol for dealing
3:37with the tape
3:38they're usually about 20 or 30 what we
3:41would call messages today but 20 or 30
3:44entries in the in that first table and
3:47the code that came along with the tape
3:49records knew how to deal with all the
3:51data so in current day terms what the
3:55Air Training Command was doing I think
3:57it was as far ago as the late 50s was to
4:01ship an entire class of objects the
4:05class was shipped with the protocol with
4:10all of the code and with all of the data
4:12in a form in which you did not have to
4:14penetrate any of it in order to make use
4:17of it that was used for a number of
4:20years as a technique until COBOL came in
4:24and the Air Force forced all of the all
4:27of those good ideas to go away in order
4:29to standardize and COBOL which is a much
4:30weaker conception going back to the
4:33notion of data structures that are
4:35fragile now I understand that we have a
4:39kind of a mixed audience today in that
